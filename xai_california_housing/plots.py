...
class Plots:
    ...
    def outlier(self, df, text, path):
        ...
        plt.savefig(f"{path}{text}.png")
        plt.show()

    def hist(self, df, x, text, path):
        self.graph_settings()
    ...
    def residplot(self, data, feature1, feature2, path):
        ...
        sb.residplot(x=data[feature1], y=data[feature2], lowess=True)
        plt.title(f'Residual Plot of {feature1} vs {feature2}')
        plt.xlabel(feature1)
        plt.ylabel(feature2)
        plt.grid(True)
        plt.savefig(f"{path}residual_{feature1}_{feature2}.png")
        plt.show()

    def scatter(self, data, feature1, feature2, path):
        plt.figure(figsize=(10, 6))
        font = FontProperties()
        font.set_family('serif bold')
        font.set_style('oblique')
        ...
        sb.scatterplot(x=data[feature1], y=data[feature2])
        plt.title(f'Scatter Plot of {feature1} vs {feature2}')
        plt.xlabel(feature1)
        plt.ylabel(feature2)
        plt.grid(True)
        plt.savefig(f"{path}scatter_{feature1}_{feature2}.png")
        plt.show()

    def scatter_old(self, df, x, y, text, path):
        self.graph_settings()
    ...
    def multi_hist(self, df, features, path='', bins=20, color='maroon', fig_size=(13, 10), grid=True):
        num_features = len(features)
        rows = (num_features + 2) // 3  # Arrange plots in rows of 3
        ...
        for i, feature in enumerate(features):
            ...
            sb.histplot(df[feature], bins=bins, kde=False, color=color)
            plt.title(feature)
            plt.grid(grid)

        plt.tight_layout()
        plt.savefig(f"{path}feature_distributions.png")
        plt.show()

    @staticmethod
    def multi_kde(df, columns, text, path):
        row = 2
        column = 3
        i = 0
        j = 0
    ...
    @staticmethod
    def plot_compare_kde(df1, df2, df3, columns, super_title, path):
        i = 0
        j = 0
        row = 2
        column = 3
        ...
        plt.savefig(f"{path}{super_title}.png")
        plt.show()

    def residual_old(self, x, y, text, x_label, y_label, path):
        self.graph_settings()
        sb.residplot(x=x, y=y, lowess=True)

        plt.title(text, fontsize=17, fontweight='bold')
    ...
    def density(self, df, x, text, path):
        self.graph_settings()

        plt.xlabel(x)
        plt.ylabel('Density')
        plt.title(text, fontsize=16, fontweight='bold')
        sb.kdeplot(df[x], fill=True)
        plt.savefig(f"{path}{text}.png")
        plt.show()
